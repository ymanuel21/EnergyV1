...
export function ProductForm({ defaultValues, brands, categories, onSubmit }: ProductFormProps) {
  ...
  // Whether the slug is still auto-generated (i.e. has never been manually edited).
  // `true` only for brand-new products (or drafts with no slug yet); editing an
  // existing product that already has a slug starts in manual mode.
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                onChange={e => {
                  ...
                  // Derive the slug in the SAME React event as the name update (React
                  // batches both dispatches), so the controlled name input never
                  // re-renders with a stale value. Once the slug is manually edited,
                  // autoSlug flips false and name changes no longer touch the slug.
                  if (autoSlug) dispatch({ type: 'SET_SLUG', value: slugify(name) });
                }}
                required
                className={inputCls}
              />
            ...
            <div>
              ...
              <SlugInput
                name="slug"
                value={state.overview.slug}
                autoGenerated={autoSlug}
                onChange={(value) => {
                  setAutoSlug(false);
                  dispatch({ type: 'SET_SLUG', value });
                }}
                className={inputCls}
              />
            </div>
